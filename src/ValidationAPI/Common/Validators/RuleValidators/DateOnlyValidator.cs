...
{
	private List<Rule>? ValidateDateOnly(string failureKey, string propertyName, RuleRequest[] rules)
	{
		...
		foreach (var rule in rules)
		{
			...
			switch (rule.Type)
			{
				...
				case RuleType.NotEqual:
					validatedRule = DateTimeValidateComparison<DateOnly>(failureKey, propertyName, rule, PropertyType.DateOnly);
					break;
				
				case RuleType.Between:
				case RuleType.Outside:
					validatedRule = DateTimeValidateRange(failureKey, rule, n => DateOnly.FromDateTime(n.DateTime));
					break;
